				...
				struct device_attribute *da, char *buf)
{
	...
	case MODULE_PRESENT_ALL:
		ret = sprintf(buf,
			...
			~(fpga_ctl->pci_fpga_dev[PCI_SUBSYSTEM_ID_UDB].qsfp_present) >> 24,
			(~(fpga_ctl->pci_fpga_dev[PCI_SUBSYSTEM_ID_UDB].qsfp_present) >> 16) & 0xFF,
			(~(fpga_ctl->pci_fpga_dev[PCI_SUBSYSTEM_ID_UDB].qsfp_present) >> 8) & 0xFF,
			~(fpga_ctl->pci_fpga_dev[PCI_SUBSYSTEM_ID_UDB].qsfp_present) & 0xFF,
			~(fpga_ctl->pci_fpga_dev[PCI_SUBSYSTEM_ID_LDB].qsfp_present) >> 24,
			(~(fpga_ctl->pci_fpga_dev[PCI_SUBSYSTEM_ID_LDB].qsfp_present) >> 16) & 0xFF,
			(~(fpga_ctl->pci_fpga_dev[PCI_SUBSYSTEM_ID_LDB].qsfp_present) >> 8) & 0xFF,
			~(fpga_ctl->pci_fpga_dev[PCI_SUBSYSTEM_ID_LDB].qsfp_present) & 0xFF,
			get_present_by_attr_index(MODULE_PRESENT_65) |
			get_present_by_attr_index(MODULE_PRESENT_66) << 1
			);
		break;
	case MODULE_LPMODE_1 ... MODULE_LPMODE_32:
		if(present)
	...
	case MODULE_RESET_1 ... MODULE_RESET_32:
		if(present)
			ret = sprintf(buf, 
				...
				(attr->index - MODULE_RESET_1)) & 0x1)?0:1);
		else
			ret = sprintf(buf, "%d\n", 0); /*unpresent: default value*/
		break;
	case MODULE_RESET_33 ... MODULE_RESET_64:
		if(present)
			ret = sprintf(buf, 
				...
				(attr->index - MODULE_RESET_33)) & 0x1)?0:1);
		else
			ret = sprintf(buf, "%d\n", 0); /*unpresent: default value*/
		break;
	case MODULE_TX_DISABLE_65:
            ...
            const char *buf, size_t count)
{
	...
	case MODULE_TX_DISABLE_65 ... MODULE_TX_DISABLE_66:
		fpga_write_port_value(PCIE_FPGA_LDB, PCIE_FPGA_SET_TX_DISABLE, 
			((attr->index - MODULE_TX_DISABLE_65) ? BIT(3) : BIT(11)), 
...
static int fpga_i2c_page_change(struct bin_attribute *attr, int page_no,
				int i2c_slave_addr)
{
	int cnt = 0;
	struct eeprom_bin_private_data *pdata = NULL;
	...
	addr = ioremap(pdata->base_addr + pdata->i2c_contrl_rtc0_config_0, 32);
	iowrite32(0x00000001|(i2c_slave_addr << 8), addr);
	iounmap(addr);
...
static ssize_t
_sfp_bin_read(struct file *filp, struct kobject *kobj,
	...
	char *buf, loff_t off, size_t count, int *page)
{
	...
	if(slice == 0) {
		if(fpga_i2c_ready_to_read(attr, EEPROM_LOWER_PAGE, 
					pdata->i2c_slave_addr) != 1)
			goto exit_emtpy;

		byte_cnt = fpga_i2c_read_data(attr, &data[0]);
	} else if(slice == 1) {
		if(fpga_i2c_ready_to_read(attr, EEPROM_UPPER_PAGE,
					pdata->i2c_slave_addr) != 1)
			goto exit_emtpy;
		...
		if(pdata->port_num <= FPGA_QSFP_PORT_NUM) { /*qsfp page1~0xff*/
			if(fpga_i2c_page_change(attr, page_num,
						pdata->i2c_slave_addr) != 1)
				goto exit_emtpy;
			...
			*page = page_num;
			...
			if(page_num == 1) { /*a2 lower page*/
				if(fpga_i2c_ready_to_read(attr,
					EEPROM_LOWER_PAGE, TWO_ADDR_0X51) != 1)
					goto exit_emtpy;

				byte_cnt = fpga_i2c_read_data(attr, &data[0]);
			}
			else if (page_num == 2) {  /*a2 page0*/
				if(fpga_i2c_page_change(attr, 0, 
						TWO_ADDR_0X51) != 1)
					goto exit_emtpy;
				...
				if(fpga_i2c_page_change(attr, (page_num-2),
	...
	char *buf, loff_t off, size_t count)
{
	...
	*/
	...
	while (count) {
		...
		status = _sfp_bin_read(filp, kobj, attr, buf, off, count, &page);
		if (status <= 0) {
			if (retval == 0)
				retval = status;
			break;
	...
	if((page > 0) && (pdata->pageable)) {
		...
		if(fpga_i2c_page_change(attr, 0, i2c_slave_addr) != 1) {
			pcie_err("ERROR: Port%d pcie get failed!!", 
				pdata->port_num);
			mutex_unlock(&update_lock);
			return -EBUSY;
		}
