...
static ssize_t show_model_name(struct device *dev, struct device_attribute *da,
			       char *buf);
static int as9736_64d_psu_read_block(struct i2c_client *client, u8 command, 
...
static struct as9736_64d_psu_data 
	*as9736_64d_psu_update_device(struct device *dev);
			   ...
			   char *buf)
{
	...
	data = as9736_64d_psu_update_device(dev);
	if (!data->valid) {
...
static struct as9736_64d_psu_data *as9736_64d_psu_update_device(
	struct device *dev)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct as9736_64d_psu_data *data = i2c_get_clientdata(client);
	...
	if (time_after(jiffies, data->last_updated + HZ + HZ / 2)
		...
		/* Read psu status */
		status = as9736_64d_cpld_read(0x60, 0x14);

		if (status < 0)
			dev_dbg(&client->dev, "cpld reg 0x60 err %d\n", status);
		else
			data->status = status & 0x3;

		status = as9736_64d_cpld_read(0x60, 0x90);

		if (status < 0) {
			dev_dbg(&client->dev, "cpld reg 0x60 err %d\n", status);
		} else {
			power_good = ((status & 0x2) >> 1) | ((status & 0x1) << 1); 
			data->status |= power_good << 2;
		}
