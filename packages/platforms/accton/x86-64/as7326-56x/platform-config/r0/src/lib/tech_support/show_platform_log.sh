...
function _show_sfp_port_status_sysfs {
    ...
    do
        ...
        port_status_cpld_i2c_bus_addr=${port_status_cpld_i2c_bus_addr_array[j]}

        # Module SFP Port RX LOS (0:Undetected, 1:Detected)
        port_rx_los=""
        sysfs="/sys/bus/i2c/devices/${port_status_cpld_i2c_bus_addr}/module_rx_los_${sfp_port_array[i]}"
        if [ -f ${sysfs} ]; then
        ...
        port_present=""
        sysfs="/sys/bus/i2c/devices/${port_status_cpld_i2c_bus_addr}/module_present_${sfp_port_array[i]}"
        if [ -f ${sysfs} ]; then
        ...
        port_tx_fault=""
        sysfs="/sys/bus/i2c/devices/${port_status_cpld_i2c_bus_addr}/module_tx_fault_${sfp_port_array[i]}"
        if [ -f ${sysfs} ]; then
        ...
        port_tx_disable=""
        sysfs="/sys/bus/i2c/devices/${port_status_cpld_i2c_bus_addr}/module_tx_disable_${sfp_port_array[i]}"
        if [ -f ${sysfs} ]; then
        ...
        if [ "${port_present}" == "1" ]; then
            sysfs="/sys/bus/i2c/devices/${sfp_eeprom_bus_array[i]}-0050/eeprom"
...
function _show_qsfpdd_port_status_sysfs {
    ...
    do
        if [ ${qsfp_port_array[i]} -le 0 ]; then
            j=${qsfp_port_array[i]#-}
            i=$((i+1))
        fi
        ...
        port_module_present=""
        sysfs="/sys/bus/i2c/devices/${port_status_cpld_i2c_bus_addr}/module_present_${qsfp_port_array[i]}"
        if [ -f ${sysfs} ]; then
        ...
        port_lp_mode=""
        sysfs="/sys/bus/i2c/devices/${port_status_cpld_i2c_bus_addr}/module_lpmode_${qsfp_port_array[i]}"
        if [ -f ${sysfs} ]; then
        ...
        port_reset=""
        sysfs="/sys/bus/i2c/devices/${port_status_cpld_i2c_bus_addr}/module_reset_${qsfp_port_array[i]}"
        if [ -f ${sysfs} ]; then
        ...
        if [ "${port_module_present}" == "1" ]; then
            sysfs="/sys/bus/i2c/devices/${qsfp_eeprom_bus_array[i]}-0050/eeprom"
            _check_filepath $sysfs
            port_eeprom_p0_1st=$(eval  "dd if=$sysfs bs=128 count=2 skip=0  status=none ${LOG_REDIRECT} | hexdump -C")
            port_eeprom_p0_2nd=$(eval  "dd if=$sysfs bs=128 count=2 skip=0  status=none ${LOG_REDIRECT} | hexdump -C")
...
function _show_bios_flash {
    ...
    if [[ ! -z "${ret}" ]]; then
        ret=$(((ret >> bios_flash_bit_field_offset) & 1))
