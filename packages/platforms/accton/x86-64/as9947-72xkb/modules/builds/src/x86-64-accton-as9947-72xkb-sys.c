...
#define DRVNAME "as9947_72xkb_sys"
...
static int as9947_72xkb_sys_probe(struct platform_device *pdev);
static int as9947_72xkb_sys_remove(struct platform_device *pdev);
...
struct as9947_72xkb_sys_data *data = NULL;

static struct platform_driver as9947_72xkb_sys_driver = {
    .probe      = as9947_72xkb_sys_probe,
    .remove     = as9947_72xkb_sys_remove,
    .driver     = {
        .name   = DRVNAME,
        .owner  = THIS_MODULE,
    },
...
static struct attribute *as9947_72xkb_sys_attributes[] = {
...
static const struct attribute_group as9947_72xkb_sys_group = {
    .attrs = as9947_72xkb_sys_attributes,
};
...
static struct as9947_72xkb_sys_data *as9947_72xkb_sys_update_cpld_ver(unsigned char cpld_addr)
{
    int status = 0;
...
static ssize_t show_cpld_version(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    data = as9947_72xkb_sys_update_cpld_ver(cpld_addr);
    if (!data->valid) {
        error = -EIO;
        goto exit;
    }
...
static int as9947_72xkb_sys_probe(struct platform_device *pdev)
{
    ...
    /* Register sysfs hooks */
    status = sysfs_create_group(&pdev->dev.kobj, &as9947_72xkb_sys_group);
    if (status)
        goto exit;
...
static int as9947_72xkb_sys_remove(struct platform_device *pdev)
{
    ...
    sysfs_remove_group(&pdev->dev.kobj, &as9947_72xkb_sys_group);

    return 0;
}

static int __init as9947_72xkb_sys_init(void)
{
    int ret;

    data = kzalloc(sizeof(struct as9947_72xkb_sys_data), GFP_KERNEL);
    if (!data) {
        ret = -ENOMEM;
        goto alloc_err;
    }
    ...
    ret = platform_driver_register(&as9947_72xkb_sys_driver);
    if (ret < 0) {
        goto dri_reg_err;
    }
...
dev_reg_err:
    platform_driver_unregister(&as9947_72xkb_sys_driver);
dri_reg_err:
    kfree(data);
alloc_err:
    return ret;
...
static void __exit as9947_72xkb_sys_exit(void)
{
...
MODULE_DESCRIPTION("as9947_72xkb_sys driver");
MODULE_LICENSE("GPL");

module_init(as9947_72xkb_sys_init);
module_exit(as9947_72xkb_sys_exit);
