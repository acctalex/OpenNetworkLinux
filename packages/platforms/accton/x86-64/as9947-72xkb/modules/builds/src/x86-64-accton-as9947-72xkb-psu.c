...
#define DRVNAME "as9947_72xkb_psu"
...
static ssize_t show_string(struct device *dev, struct device_attribute *attr,
                            char *buf);
static int as9947_72xkb_psu_probe(struct platform_device *pdev);
static int as9947_72xkb_psu_remove(struct platform_device *pdev);
...
struct as9947_72xkb_psu_data *data = NULL;

static struct platform_driver as9947_72xkb_psu_driver = {
    .probe = as9947_72xkb_psu_probe,
    .remove = as9947_72xkb_psu_remove,
    .driver = {
        .name = DRVNAME,
        .owner = THIS_MODULE,
    },
...
enum as9947_72xkb_psu_sysfs_attrs {
    /* psu attributes */
    PSU_ATTR(1),
    PSU_ATTR(2),
    NUM_OF_PSU_ATTR,
...
static struct attribute *as9947_72xkb_psu1_attrs[] = {
    /* psu attributes */
    DECLARE_PSU_ATTR(1),
...
static struct attribute_group as9947_72xkb_psu1_group = {
    .attrs = as9947_72xkb_psu1_attrs,
};
/* ATTRIBUTE_GROUPS(as9947_72xkb_psu1); */
...
static struct attribute *as9947_72xkb_psu2_attrs[] = {
    /* psu attributes */
    DECLARE_PSU_ATTR(2),
...
static struct attribute_group as9947_72xkb_psu2_group = {
    .attrs = as9947_72xkb_psu2_attrs,
};
/* ATTRIBUTE_GROUPS(as9947_72xkb_psu2); */

const struct attribute_group *as9947_72xkb_psu_groups[][2] = {
    {&as9947_72xkb_psu1_group, NULL},
    {&as9947_72xkb_psu2_group, NULL}
};
                                ...
                                unsigned char *rx_data, unsigned short rx_len)
{
    ...
    for (retry = 0; retry <= IPMI_ERR_RETRY_TIMES; retry++) {
        ...
        if (unlikely(status != 0)) {
            pr_err("ipmi_send_message_%d err status(%d)[%s] raw_cmd=[%s] tx_msgid=(%02x)\r\n",
        ...
        if (unlikely(ipmi->rx_result != 0)) {
            pr_err("ipmi_send_message_%d err result(%d)[%s] raw_cmd=[%s] tx_msgid=(%02x)\r\n",
...
static struct as9947_72xkb_psu_data *as9947_72xkb_psu_update_device(struct device_attribute *da)
{
                            ...
                            char *buf)
{
    ...
    data = as9947_72xkb_psu_update_device(da);
    if (!data->valid[pid]) {
        error = -EIO;
        goto exit;
    }
...
static int as9947_72xkb_psu_probe(struct platform_device *pdev)
{
    ...
    hwmon_dev = hwmon_device_register_with_info(&pdev->dev, DRVNAME, 
                    NULL, NULL, as9947_72xkb_psu_groups[pdev->id]);
    if (IS_ERR(hwmon_dev)) {
...
static int __init as9947_72xkb_psu_init(void)
{
    int ret;
    int i;

    data = kzalloc(sizeof(struct as9947_72xkb_psu_data), GFP_KERNEL);
    if (!data) {
        ret = -ENOMEM;
        goto alloc_err;
    }
    ...
    ret = platform_driver_register(&as9947_72xkb_psu_driver);
    if (ret < 0)
        goto dri_reg_err;
...
dev_reg_err:
    platform_driver_unregister(&as9947_72xkb_psu_driver);
dri_reg_err:
    kfree(data);
alloc_err:
    return ret;
...
static void __exit as9947_72xkb_psu_exit(void)
{
    int i;
...
MODULE_DESCRIPTION("as9947_72xkb_psu driver");
MODULE_LICENSE("GPL");

module_init(as9947_72xkb_psu_init);
module_exit(as9947_72xkb_psu_exit);
