...
static struct as9817_32_psu_data *as9817_32_psu_update_device(struct device_attribute *da)
{
                            ...
                            char *buf)
{
    ...
    data = as9817_32_psu_update_device(da);
    if (!data->valid[pid]) {
        error = -EIO;
        goto exit;
    }
...
static int as9817_32_psu_probe(struct platform_device *pdev)
{
    ...
    hwmon_dev = hwmon_device_register_with_info(&pdev->dev, DRVNAME, 
                    NULL, NULL, as9817_32_psu_groups[pdev->id]);
    if (IS_ERR(hwmon_dev)) {
