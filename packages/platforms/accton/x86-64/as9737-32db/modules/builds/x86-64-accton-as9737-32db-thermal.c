				...
				unsigned char *rx_data, unsigned short rx_len)
{
	...
	for (retry = 0; retry <= IPMI_ERR_RETRY_TIMES; retry++) {
		...
		if (unlikely(status != 0)) {
			dev_err(&data->pdev->dev, "ipmi cmd(%x) err status(%d)[%s]\r\n",
				cmd, status, cmdline ? cmdline : "");
			continue;
		}

		if (unlikely(ipmi->rx_result != 0)) {
			dev_err(&data->pdev->dev, "ipmi cmd(%x) err result(%d)[%s]\r\n",
				cmd, ipmi->rx_result, cmdline ? cmdline : "");
			continue;
		}
