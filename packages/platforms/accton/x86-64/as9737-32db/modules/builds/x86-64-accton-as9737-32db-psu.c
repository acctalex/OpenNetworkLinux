...
#define DECLARE_PSU_SENSOR_DEVICE_ATTR(index) \
	...
	static SENSOR_DEVICE_ATTR(psu##index##_pout, S_IRUGO, show_psu, NULL, \
					PSU##index##_POUT); \
	static SENSOR_DEVICE_ATTR(psu##index##_model, S_IRUGO, show_string, \
					NULL, PSU##index##_MODEL); \
	static SENSOR_DEVICE_ATTR(psu##index##_serial, S_IRUGO, show_string, \
					NULL, PSU##index##_SERIAL); \
	static SENSOR_DEVICE_ATTR(psu##index##_temp1_input, S_IRUGO, show_psu, \
					NULL, PSU##index##_TEMP1_INPUT); \
	static SENSOR_DEVICE_ATTR(psu##index##_temp2_input, S_IRUGO, show_psu, \
					NULL, PSU##index##_TEMP2_INPUT); \
	static SENSOR_DEVICE_ATTR(psu##index##_temp3_input, S_IRUGO, show_psu, \
					NULL, PSU##index##_TEMP3_INPUT); \
	static SENSOR_DEVICE_ATTR(psu##index##_fan1_input, S_IRUGO, show_psu, \
					NULL, PSU##index##_FAN_INPUT); \
	static SENSOR_DEVICE_ATTR(psu##index##_fan_dir, S_IRUGO, show_string, \
					NULL, PSU##index##_FAN_DIR); \
				...
				unsigned char *rx_data, unsigned short rx_len)
{
	...
	for (retry = 0; retry <= IPMI_ERR_RETRY_TIMES; retry++) {
		...
		if (unlikely(status != 0)) {
			dev_err(dev, 
				"ipmi_send_message_%d err status(%d)[%s]\r\n", 
		...
		if (unlikely(ipmi->rx_result != 0)) {
			dev_err(dev, 
				"ipmi_send_message_%d err result(%d)[%s]\r\n", 
				retry, ipmi->rx_result, cmdline ? cmdline : "");
			continue;
		}
...
static struct as9737_32db_psu_data *as9737_32db_psu_update_device(
...
static int __init as9737_32db_psu_init(void)
{
	...
	for (i = 0; i < NUM_OF_PSU; i++) {
		data->pdev[i] = platform_device_register_simple(DRVNAME, i, 
				NULL, 0);
