...
static const int front_fan_target_speed_f2b[] = {
	0,  6466,  8200,  10000,  11800,  13500,  15266, 16933,
	18766, 20433, 22200, 24000, 25733, 27400, 29233, 30833
};

static const int rear_fan_target_speed_f2b[] = {
	0,  5200,  6600,  8000,  9466,  10833,  12266, 13666,
	15066, 16500, 17866, 19300, 20733, 22133, 23566, 24900
};
...
#define DECLARE_FAN_SENSOR_DEVICE_ATTR(index) \
	...
	static SENSOR_DEVICE_ATTR(fan##index##_input, S_IRUGO, show_fan, \ 
				NULL, FAN##index##_INPUT); \
	static SENSOR_DEVICE_ATTR(fan##index##_dir, S_IRUGO, show_dir, NULL, \
				FAN##index##_DIR); \
	static SENSOR_DEVICE_ATTR(fan##index##_fault, S_IRUGO, show_fan, \ 
				NULL, FAN##index##_FAULT); \
	static SENSOR_DEVICE_ATTR(fan##index##_target, S_IRUGO, \ 
				show_threshold, NULL, FAN##index##_TARGET); \
	static SENSOR_DEVICE_ATTR(fan##index##_tolerance, S_IRUGO, \ 
				show_threshold, NULL, FAN##index##_TOLERANCE)
...
static struct as9737_32db_fan_data *as9737_32db_fan_update_device(void)
{
	...
	/* BMC not yet ready for this command */
#if 0
				...
				char *buf)
{
	...
	case FAN12_PWM:
		...
		value = DIV_ROUND_CLOSEST(
			data->ipmi_resp[index + FAN_PWM] * 625, 100);
		break;
	case FAN1_INPUT:
	case FAN2_INPUT:
	case FAN3_INPUT:
		...
		const char *buf, size_t count)
{
	...
	pwm = (pwm * 100) / 625; /* Convert pwm to register value */
...
#if 1
		index = (fid % NUM_OF_FAN_MODULE) * FAN_DATA_COUNT;
		if (attr->index < FAN7_TARGET)
			value = front_fan_target_speed_f2b[data->ipmi_resp[
				index + FAN_PWM]];
		else
			value = rear_fan_target_speed_f2b[data->ipmi_resp[
				index + FAN_PWM]];
#else
