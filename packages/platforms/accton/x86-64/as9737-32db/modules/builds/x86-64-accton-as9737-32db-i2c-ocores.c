	...
	*/
	...
	if ((i2c->state == STATE_START) || (i2c->state == STATE_WRITE)) {
		...
		if (stat & OCI2C_STAT_NACK) {
			...
			if (debug)
				dev_warn(dev, 
					 "I2C %s, no ACK from slave 0x%02x (%s)",
					 i2c->adap.name, 
					 msg->addr, cmdline ? cmdline : "");
...
static int ocores_i2c_probe(struct platform_device *pdev)
{
...
#endif
		dev_info(&pdev->dev, 
			 "Resouce Memory: start:0x%llx, end:0x%llx", 
			 res->start, res->end);
	...
	if (pdata) {
		...
		dev_info(&pdev->dev, 
			 "Write %d KHz, ioWidth:%d, shift:%d", 
			 i2c->ip_clock_khz, 
			 pdata->reg_io_width ,pdata->reg_shift);
		i2c->bus_clock_khz = 100;
	} else {
...
MODULE_LICENSE("GPL");
MODULE_ALIAS("platform:ocores-as9737");
