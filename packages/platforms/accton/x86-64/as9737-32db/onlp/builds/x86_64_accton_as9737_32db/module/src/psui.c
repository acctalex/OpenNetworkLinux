...
onlp_psui_info_get(onlp_oid_t id, onlp_psu_info_t* info)
{
	...
	*info = pinfo[pid]; /* Set the onlp_oid_hdr_t */
	...
	ret = onlp_file_read_int(&val, 
				 PSU_SYSFS_FORMAT, (pid-1), pid, "present");
	...
	ret = onlp_file_read_int(&val, 
				 PSU_SYSFS_FORMAT, (pid-1), pid, "power_good");
	if (ret < 0) {
	...
	info->hdr.coids[0] = ONLP_THERMAL_ID_CREATE(thermal_count + 
				(pid-1)*NUM_OF_THERMAL_PER_PSU + 1);
	info->hdr.coids[1] = ONLP_THERMAL_ID_CREATE(thermal_count + 
				(pid-1)*NUM_OF_THERMAL_PER_PSU + 2);
	info->hdr.coids[2] = ONLP_THERMAL_ID_CREATE(thermal_count + 
	...
	if (hwmon_idx >= 0) {
		...
		len = onlp_file_read_str(&str, 
				PSU_SYSFS_FORMAT_1, (pid-1), hwmon_idx, file);
