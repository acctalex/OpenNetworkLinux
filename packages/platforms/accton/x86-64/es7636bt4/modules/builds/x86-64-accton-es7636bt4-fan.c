/*
 * A hwmon driver for the Accton es7636bt4 fan
 *
...
#define DRVNAME "es7636bt4_fan"

static struct es7636bt4_fan_data *es7636bt4_fan_update_device(
						struct device *dev);
...
static const u8 fan_reg[] = {
	...
	0x12,       /* front fan 6 speed(rpm) */
	0x13,       /* front fan 5 speed(rpm) */
	0x14,       /* front fan 4 speed(rpm) */
	0x15,       /* front fan 3 speed(rpm) */
	0x16,       /* front fan 2 speed(rpm) */
	0x17,       /* front fan 1 speed(rpm) */
	0x22,       /* rear fan 6 speed(rpm) */
	0x23,       /* rear fan 5 speed(rpm) */
	0x24,       /* rear fan 4 speed(rpm) */
	0x25,       /* rear fan 3 speed(rpm) */
	0x26,       /* rear fan 2 speed(rpm) */
	0x27,       /* rear fan 1 speed(rpm) */
};

/* Each client has this additional data */
struct es7636bt4_fan_data {
...
static struct attribute *es7636bt4_fan_attributes[] = {
...
static int es7636bt4_fan_read_value(struct i2c_client *client, u8 reg)
{
	return i2c_smbus_read_byte_data(client, reg);
}

static int es7636bt4_fan_write_value(struct i2c_client *client, u8 reg,
					u8 value)
{
...
static u8 duty_cycle_to_reg_val(u8 duty_cycle)
{
	if (duty_cycle == 0)
		return 0;
	else if(duty_cycle > 0 && duty_cycle < 12)
		return 1;
	else
...
static u8 is_fan_fault(struct es7636bt4_fan_data *data, enum fan_id id)
{
	u8 ret = 1;
	int front_fan_index = FAN6_FRONT_SPEED_RPM + id;
	int rear_fan_index  = FAN6_REAR_SPEED_RPM  + id;
			      ...
			      const char *buf, size_t count)
{
	...
	struct es7636bt4_fan_data *data = i2c_get_clientdata(client);
	 ...
	 */
	error = es7636bt4_fan_write_value(client, 0x33, 0);
	if (error != 0) {
	...
	es7636bt4_fan_write_value(client, 
			      ...
			      char *buf)
{
	...
	mutex_lock(&data->update_lock);
	data = es7636bt4_fan_update_device(dev);
	...
	if (data->valid) {
		...
		case FAN6_FAULT:
			ret = sprintf(buf, "%d\n", 
				is_fan_fault(data, FAN6_FAULT - attr->index));
			break;
		case FAN1_DIRECTION:
		case FAN2_DIRECTION:
		case FAN3_DIRECTION:
...
static const struct attribute_group es7636bt4_fan_group = {
	.attrs = es7636bt4_fan_attributes,
};

static struct es7636bt4_fan_data *es7636bt4_fan_update_device(struct 
								device *dev)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct es7636bt4_fan_data *data = i2c_get_clientdata(client);
	...
	if (time_after(jiffies, data->last_updated + HZ + HZ / 2) ||
		...
		dev_dbg(&client->dev, "Starting es7636bt4_fan update\n");
		data->valid = 0;

		/* Update fan data
		 */
		for (i = 0; i < ARRAY_SIZE(data->reg_val); i++) {
			int status = es7636bt4_fan_read_value(client, 
								fan_reg[i]);
			if (status < 0) {
				data->valid = 0;
				mutex_unlock(&data->update_lock);
...
static int es7636bt4_fan_probe(struct i2c_client *client,
				const struct i2c_device_id *dev_id)
{
	struct es7636bt4_fan_data *data;
	int status;
	...
	data = kzalloc(sizeof(struct es7636bt4_fan_data), GFP_KERNEL);
	if (!data) {
		status = -ENOMEM;
		goto exit;
	}
	...
	status = sysfs_create_group(&client->dev.kobj, &es7636bt4_fan_group);
	if (status)
		goto exit_free;
...
exit_remove:
	sysfs_remove_group(&client->dev.kobj, &es7636bt4_fan_group);
exit_free:
	kfree(data);
exit:
...
static int es7636bt4_fan_remove(struct i2c_client *client)
{
	struct es7636bt4_fan_data *data = i2c_get_clientdata(client);
...
static const struct i2c_device_id es7636bt4_fan_id[] = {
	{ "es7636bt4_fan", 0 },
	{}
};
MODULE_DEVICE_TABLE(i2c, es7636bt4_fan_id);

static struct i2c_driver es7636bt4_fan_driver = {
	.class    = I2C_CLASS_HWMON,
	.driver = {
		.name     = DRVNAME,
	},
	.probe        = es7636bt4_fan_probe,
	.remove       = es7636bt4_fan_remove,
	.id_table     = es7636bt4_fan_id,
	.address_list = normal_i2c,
};

module_i2c_driver(es7636bt4_fan_driver);

MODULE_AUTHOR("Alex Lai <alex_lai@edge-core.com>");
MODULE_DESCRIPTION("es7636bt4_fan driver");
MODULE_LICENSE("GPL");
