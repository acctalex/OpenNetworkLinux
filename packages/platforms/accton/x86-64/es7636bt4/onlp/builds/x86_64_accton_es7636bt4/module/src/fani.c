...
{ \
	{ ONLP_FAN_ID_CREATE(FAN_##fid##_ON_FAN_BOARD), \
	  "Chassis Fan - "#fid, 0 },\
	0x0, \
...
{ \
	{ ONLP_FAN_ID_CREATE(FAN_##fid##_ON_PSU_##pid), \
	  "PSU "#pid" - Fan "#fid, 0 }, \
	0x0, \
...
static int _onlp_fani_info_get_fan(int fid, onlp_fan_info_t* info)
{
	...
	/* get rear fan speed
	 */
	sprintf(path, "%s""fan%d_rear_speed_rpm", FAN_BOARD_PATH, 
		fid);
...
static uint32_t _onlp_get_fan_direction_on_psu(void)
{
	...
	for (i = PSU1_ID; i <= PSU2_ID; i++) {
		...
		if (psu_status_info_get(i, "psu_power_good", &val) != 0)
...
int onlp_fani_percentage_set(onlp_oid_t id, int p)
{
	...
	switch (fid) {
	case FAN_1_ON_PSU_1:
		return psu_ym2651y_pmbus_info_set(PSU1_ID,
					"psu_fan_duty_cycle_percentage", p);
	case FAN_1_ON_PSU_2:
		return psu_ym2651y_pmbus_info_set(PSU2_ID,
					"psu_fan_duty_cycle_percentage", p);
	case FAN_1_ON_FAN_BOARD:
	case FAN_2_ON_FAN_BOARD:
	case FAN_3_ON_FAN_BOARD:
