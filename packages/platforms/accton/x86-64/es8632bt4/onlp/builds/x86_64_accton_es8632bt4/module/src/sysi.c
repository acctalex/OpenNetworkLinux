...
#include "x86_64_accton_es8632bt4_int.h"
#include "x86_64_accton_es8632bt4_log.h"
...
{
	return "x86-64-accton-es8632bt4-r0";
}
...
fan_ctrl_policy_t  fan_thermal_policy_f2b[] = {
	{38,  0x5, 0,     38000,   LEVEL_FAN_DEF},
	{63,  0x9, 38000, 46000,   LEVEL_FAN_MID},
	{100, 0xF, 46000, 58000,   LEVEL_FAN_MAX},
	{100, 0xF, 58000, 66000,   LEVEL_TEMP_HIGH},
	{100, 0xF, 66000, 200000,  LEVEL_TEMP_CRITICAL}
};

fan_ctrl_policy_t  fan_thermal_policy_b2f[] = {
	{38,  0x5, 0,     34000,   LEVEL_FAN_DEF},
	{63,  0x9, 34000, 44000,   LEVEL_FAN_MID},
	{100, 0xF, 44000, 59000,   LEVEL_FAN_MAX},
	{100, 0xF, 59000, 67000,   LEVEL_TEMP_HIGH},
	{100, 0xF, 67000, 200000,  LEVEL_TEMP_CRITICAL}
};
...
int onlp_sysi_platform_manage_fans(void)
{
	...
	onlp_thermal_info_t thermal_1, thermal_2, thermal_3, thermal_4, 
			    thermal_5, thermal_6, thermal_7;
	char  buf[10] = {0};
	 ...
	 */
	if (onlp_thermali_info_get(ONLP_THERMAL_ID_CREATE(1), &thermal_1) != 
	    ONLP_STATUS_OK) {
	...
	if (onlp_thermali_info_get(ONLP_THERMAL_ID_CREATE(2), &thermal_2) != 
	    ONLP_STATUS_OK) {
		AIM_LOG_ERROR("Unable to read thermal status, set fans to 75 %% speed");
		onlp_fani_percentage_set(ONLP_FAN_ID_CREATE(1), 
				fan_thermal_policy[LEVEL_FAN_MID].duty_cycle);
	...
	if (onlp_thermali_info_get(ONLP_THERMAL_ID_CREATE(3), &thermal_3) != 
	    ONLP_STATUS_OK) {
		AIM_LOG_ERROR("Unable to read thermal status, set fans to 75 %% speed");
		onlp_fani_percentage_set(ONLP_FAN_ID_CREATE(1), 
				fan_thermal_policy[LEVEL_FAN_MID].duty_cycle);
	...
	if (onlp_thermali_info_get(ONLP_THERMAL_ID_CREATE(6), &thermal_6) != 
	    ONLP_STATUS_OK) {
		AIM_LOG_ERROR("Unable to read thermal status, set fans to 75 %% speed");
		onlp_fani_percentage_set(ONLP_FAN_ID_CREATE(1), 
				fan_thermal_policy[LEVEL_FAN_MID].duty_cycle);
	...
	if (onlp_thermali_info_get(ONLP_THERMAL_ID_CREATE(7), &thermal_7) != 
	    ONLP_STATUS_OK) {
		AIM_LOG_ERROR("Unable to read thermal status, set fans to 75 %% speed");
		onlp_fani_percentage_set(ONLP_FAN_ID_CREATE(1), 
				fan_thermal_policy[LEVEL_FAN_MID].duty_cycle);
	...
	temp = (thermal_2.mcelsius + thermal_3.mcelsius + thermal_4.mcelsius + 
		thermal_5.mcelsius + thermal_6.mcelsius + thermal_7.mcelsius) / 6;

	temp = (temp + thermal_1.mcelsius) / 2;
	...
	for (i = 0; 
	     i < sizeof(fan_thermal_policy_f2b) / sizeof(fan_ctrl_policy_t); 
	     i++) {
