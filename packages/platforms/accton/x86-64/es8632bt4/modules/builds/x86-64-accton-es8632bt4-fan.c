...
#define DRVNAME "es8632bt4_fan"

static struct es8632bt4_fan_data *es8632bt4_fan_update_device(
						struct device *dev);
...
static struct attribute *es8632bt4_fan_attributes[] = {
...
static int es8632bt4_fan_read_value(struct i2c_client *client, u8 reg)
{
	return i2c_smbus_read_byte_data(client, reg);
}

static int es8632bt4_fan_write_value(struct i2c_client *client, u8 reg,
					u8 value)
{
...
static u8 is_fan_fault(struct es8632bt4_fan_data *data, enum fan_id id)
{
	u8 ret = 1;
			      ...
			      const char *buf, size_t count)
{
	...
	struct es8632bt4_fan_data *data = i2c_get_clientdata(client);
	 ...
	 */
	error = es8632bt4_fan_write_value(client, 0x33, 0);
	if (error != 0) {
	...
	es8632bt4_fan_write_value(client, 
			      ...
			      char *buf)
{
	...
	mutex_lock(&data->update_lock);
	data = es8632bt4_fan_update_device(dev);
...
static const struct attribute_group es8632bt4_fan_group = {
	.attrs = es8632bt4_fan_attributes,
};

static struct es8632bt4_fan_data *es8632bt4_fan_update_device(struct 
								device *dev)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct es8632bt4_fan_data *data = i2c_get_clientdata(client);
	...
	if (time_after(jiffies, data->last_updated + HZ + HZ / 2) ||
		...
		dev_dbg(&client->dev, "Starting es8632bt4_fan update\n");
		data->valid = 0;

		/* Update fan data
		 */
		for (i = 0; i < ARRAY_SIZE(data->reg_val); i++) {
			int status = es8632bt4_fan_read_value(client, 
								fan_reg[i]);
			if (status < 0) {
				data->valid = 0;
				mutex_unlock(&data->update_lock);
...
static int es8632bt4_fan_probe(struct i2c_client *client,
				const struct i2c_device_id *dev_id)
{
	struct es8632bt4_fan_data *data;
	int status;
	...
	data = kzalloc(sizeof(struct es8632bt4_fan_data), GFP_KERNEL);
	if (!data) {
		status = -ENOMEM;
		goto exit;
	}
	...
	status = sysfs_create_group(&client->dev.kobj, &es8632bt4_fan_group);
	if (status)
		goto exit_free;
...
exit_remove:
	sysfs_remove_group(&client->dev.kobj, &es8632bt4_fan_group);
exit_free:
	kfree(data);
exit:
...
static int es8632bt4_fan_remove(struct i2c_client *client)
{
	struct es8632bt4_fan_data *data = i2c_get_clientdata(client);
...
static const struct i2c_device_id es8632bt4_fan_id[] = {
	{ "es8632bt4_fan", 0 },
	{}
};
MODULE_DEVICE_TABLE(i2c, es8632bt4_fan_id);

static struct i2c_driver es8632bt4_fan_driver = {
	.class    = I2C_CLASS_HWMON,
	.driver = {
		.name     = DRVNAME,
	},
	.probe        = es8632bt4_fan_probe,
	.remove       = es8632bt4_fan_remove,
	.id_table     = es8632bt4_fan_id,
	.address_list = normal_i2c,
};

module_i2c_driver(es8632bt4_fan_driver);

MODULE_AUTHOR("Alex Lai <alex_lai@edge-core.com>");
MODULE_DESCRIPTION("es8632bt4_fan driver");
MODULE_LICENSE("GPL");
