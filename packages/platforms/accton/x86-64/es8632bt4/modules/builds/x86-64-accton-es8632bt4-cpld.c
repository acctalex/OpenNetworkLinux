 ...
 * This includes the:
 *	 Accton es8632bt4 CPLD1/CPLD2/CPLD3
 *
 * Based on:
...
struct es8632bt4_cpld_data {
	enum cpld_type   type;
	struct device   *hwmon_dev;
	struct mutex     update_lock;
};

static const struct i2c_device_id es8632bt4_cpld_id[] = {
	{ "es8632bt4_fpga", es8632bt4_fpga },
	{ "es8632bt4_cpld1", es8632bt4_cpld1 },
	{ "es8632bt4_cpld2", es8632bt4_cpld2 },
	{ "es8632bt4_cpld_cpu", es8632bt4_cpld_cpu },
	{ }
};
MODULE_DEVICE_TABLE(i2c, es8632bt4_cpld_id);
...
enum es8632bt4_cpld_sysfs_attributes {
	CPLD_VERSION,
	ACCESS,
	MODULE_PRESENT_ALL,
	MODULE_RXLOS_ALL,
...
static ssize_t show_version(struct device *dev, struct device_attribute *da,
			char *buf);
static int es8632bt4_cpld_read_internal(struct i2c_client *client, u8 reg);
static int es8632bt4_cpld_write_internal(struct i2c_client *client, u8 reg,
					u8 value);
...
static struct attribute *es8632bt4_fpga_attributes[] = {
	&sensor_dev_attr_version.dev_attr.attr,
	&sensor_dev_attr_access.dev_attr.attr,
	NULL
};

static const struct attribute_group es8632bt4_fpga_group = {
	.attrs = es8632bt4_fpga_attributes,
};

static struct attribute *es8632bt4_cpld1_attributes[] = {
	&sensor_dev_attr_version.dev_attr.attr,
	&sensor_dev_attr_access.dev_attr.attr,
...
static const struct attribute_group es8632bt4_cpld1_group = {
	.attrs = es8632bt4_cpld1_attributes,
};

static struct attribute *es8632bt4_cpld2_attributes[] = {
	&sensor_dev_attr_version.dev_attr.attr,
	&sensor_dev_attr_access.dev_attr.attr,
	&sensor_dev_attr_module_present_all.dev_attr.attr,
...
static const struct attribute_group es8632bt4_cpld2_group = {
	.attrs = es8632bt4_cpld2_attributes,
};
				...
				struct device_attribute *da, char *buf)
{
	...
	struct  es8632bt4_cpld_data *data = i2c_get_clientdata(client);


	if (data->type == es8632bt4_cpld1) {
		u8 regs[] = {0x12, 0x13};
		u8 values[2] = {0};

		mutex_lock(&data->update_lock);
		for (i = 0; i < ARRAY_SIZE(regs); i++) {
			status = es8632bt4_cpld_read_internal(client, regs[i]);
	...
	} else if (data->type == es8632bt4_cpld2) {
			      ...
			      char *buf)
{
	...
	status = es8632bt4_cpld_read_internal(client, reg);
			...
			const char *buf, size_t count)
{
	...
	status = es8632bt4_cpld_write_internal(client, reg, status);
	if (unlikely(status < 0))
		goto exit;
			...
			const char *buf, size_t count)
{
	...
	mutex_lock(&data->update_lock);
	status = es8632bt4_cpld_write_internal(client, addr, val);
	if (unlikely(status < 0))
		goto exit;
...
static void es8632bt4_cpld_add_client(struct i2c_client *client)
{
...
static void es8632bt4_cpld_remove_client(struct i2c_client *client)
{
 ...
 */
static int es8632bt4_cpld_probe(struct i2c_client *client,
			 const struct i2c_device_id *id)
{
	struct i2c_adapter *adap = to_i2c_adapter(client->dev.parent);
	struct es8632bt4_cpld_data *data;
	int ret = -ENODEV;
	int status;	
	...
	data = kzalloc(sizeof(struct es8632bt4_cpld_data), GFP_KERNEL);
	if (!data) {
		ret = -ENOMEM;
		goto exit;
	}
	...
	switch (data->type) {
	case es8632bt4_fpga:
		group = &es8632bt4_fpga_group;
		break;
	case es8632bt4_cpld1:
		group = &es8632bt4_cpld1_group;
		break;
	case es8632bt4_cpld2:
		group = &es8632bt4_cpld2_group;
		break; 
	case es8632bt4_cpld_cpu:
	/* Disable CPLD reset to avoid DUT will be reset.
	 */
	status = es8632bt4_cpld_write_internal(client, 0x3, 0x0); 
	if (status < 0)
	...
	es8632bt4_cpld_add_client(client);
	return 0;
...
static int es8632bt4_cpld_remove(struct i2c_client *client)
{
	struct es8632bt4_cpld_data *data = i2c_get_clientdata(client);
	const struct attribute_group *group = NULL;

	es8632bt4_cpld_remove_client(client);
...
int es8632bt4_cpld_read(unsigned short cpld_addr, u8 reg)
{
	struct list_head   *list_node = NULL;
	struct cpld_client_node *cpld_node = NULL;
	int ret = -EPERM;
	...
	list_for_each(list_node, &cpld_client_list)
	{
		...
		if (cpld_node->client->addr == cpld_addr) {
			ret = es8632bt4_cpld_read_internal(cpld_node->client,
								reg);
			break;
		}
	}
...
EXPORT_SYMBOL(es8632bt4_cpld_read);

int es8632bt4_cpld_write(unsigned short cpld_addr, u8 reg, u8 value)
{
	struct list_head   *list_node = NULL;
	struct cpld_client_node *cpld_node = NULL;
	int ret = -EIO;
	...
	list_for_each(list_node, &cpld_client_list)
	{
		cpld_node = list_entry(list_node, struct cpld_client_node,
					list);
		if (cpld_node->client->addr == cpld_addr) {
			ret = es8632bt4_cpld_write_internal(cpld_node->client,
								reg, value);
			break;
		}
	}
...
EXPORT_SYMBOL(es8632bt4_cpld_write);

static struct i2c_driver es8632bt4_cpld_driver = {
	.driver		= {
		.name	= "es8632bt4_cpld",
		.owner	= THIS_MODULE,
	},
	.probe		= es8632bt4_cpld_probe,
	.remove		= es8632bt4_cpld_remove,
	.id_table	= es8632bt4_cpld_id,
};

static int __init es8632bt4_cpld_init(void)
{
	mutex_init(&list_lock);
	return i2c_add_driver(&es8632bt4_cpld_driver);
}

static void __exit es8632bt4_cpld_exit(void)
{
	i2c_del_driver(&es8632bt4_cpld_driver);
}
...
module_init(es8632bt4_cpld_init);
module_exit(es8632bt4_cpld_exit);
