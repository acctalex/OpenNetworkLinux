...
int onlp_get_psu_hwmon_idx(int pid)
{
    ...
    for (hwmon_idx = 0; hwmon_idx <= max_hwmon_idx; hwmon_idx++) {
        snprintf(path, sizeof(path), "/sys/devices/platform/as9957_32db_psu/hwmon/hwmon%d/", hwmon_idx);
...
int onlp_get_fan_hwmon_idx(void)
{
    ...
    for (hwmon_idx = 0; hwmon_idx <= max_hwmon_idx; hwmon_idx++) {
        snprintf(path, sizeof(path), "/sys/devices/platform/as9957_32db_fan/hwmon/hwmon%d/", hwmon_idx);
...
int psu_status_info_get(int id, char *node, int *value)
{
    ...
    *value = 0;
    ...
    if (strncmp(node, "fan_dir", strlen("fan_dir")) == 0) {
        ...
        snprintf(path, sizeof(path), PSU_SYSFS_FORMAT, id, node);
