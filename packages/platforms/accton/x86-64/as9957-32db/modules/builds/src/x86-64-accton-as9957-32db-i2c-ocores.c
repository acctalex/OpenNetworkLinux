...
MODULE_PARM_DESC(timeout, "Timeout for ocores_poll_wait, in unit of milliseconds.");
...
static void ocores_process(struct ocores_i2c *i2c, u8 stat)
{
    ...
    if ((i2c->state == STATE_START) || (i2c->state == STATE_WRITE)) {
        ...
        if (stat & OCI2C_STAT_NACK) {
            i2c->state = STATE_ERROR;
            if (debug) {
                dev_warn(dev, "I2C %s, no ACK from slave 0x%02x (%s)",
                         i2c->adap.name, msg->addr, cmdline ? cmdline : "");
            }
            oc_setreg(i2c, OCI2C_CMD, OCI2C_CMD_STOP);
            goto out;
        }
...
#else
static int __init ocores_i2c_as9957_32db_init(void)
{
    int err;
...
static void __exit ocores_i2c_as9957_32db_exit(void)
{
    platform_driver_unregister(&ocores_i2c_driver);
}

module_init(ocores_i2c_as9957_32db_init);
module_exit(ocores_i2c_as9957_32db_exit);
#endif

MODULE_AUTHOR("Peter Korsgaard <peter@korsgaard.com>");
MODULE_DESCRIPTION("OpenCores I2C bus driver");
MODULE_LICENSE("GPL");
MODULE_ALIAS("platform:ocores-as9957");
