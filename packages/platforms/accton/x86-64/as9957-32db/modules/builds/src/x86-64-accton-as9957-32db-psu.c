...
#define DRVNAME "as9957_32db_psu"
...
static ssize_t show_string(struct device *dev, struct device_attribute *attr,
                            char *buf);
static ssize_t set_max_fan_speed(struct device *dev, struct device_attribute *da,
               const char *buf, size_t count);
static int as9957_32db_psu_probe(struct platform_device *pdev);
static int as9957_32db_psu_remove(struct platform_device *pdev);

struct max_fan_speed_info {
    char *model_name;
    u8 length;
    u32 max_fan_speed;
};

struct max_fan_speed_info psu_max_fan_speed[] = {
    {"PS-2302-6L", 10, 29952},
    {"DD-2302-2L", 10, 33000},
    {"R1CA2302A", 9, 32024},
};
...
struct ipmi_psu_resp_data {
    ...
    unsigned max_fan_speed;
};

struct as9957_32db_psu_data {
...
struct as9957_32db_psu_data *data = NULL;

static struct platform_driver as9957_32db_psu_driver = {
    .probe = as9957_32db_psu_probe,
    .remove = as9957_32db_psu_remove,
    .driver = {
        .name = DRVNAME,
        .owner = THIS_MODULE,
    },
...
#define PSU_FAN_INPUT_MAX_ATTR_ID(index) PSU##index##_FAN_INPUT_MAX
...
#define PSU_ATTR(psu_id) \
    ...
    PSU_FAN_INPUT_MAX_ATTR_ID(psu_id), \
...
enum as9957_32db_psu_sysfs_attrs {
    /* psu attributes */
    PSU_ATTR(1),
    PSU_ATTR(2),
    NUM_OF_PSU_ATTR,
...
#define DECLARE_PSU_SENSOR_DEVICE_ATTR(index) \
    ...
    static SENSOR_DEVICE_ATTR(psu##index##_fan1_input_max, S_IWUSR | S_IRUGO, show_psu, set_max_fan_speed,\
                                PSU##index##_FAN_INPUT_MAX); \
    static SENSOR_DEVICE_ATTR(psu##index##_fan_dir, S_IRUGO, show_string, NULL,\
                                PSU##index##_FAN_DIR); \
    static SENSOR_DEVICE_ATTR(psu##index##_type, S_IRUGO, show_psu_info, NULL,\
                                PSU##index##_TYPE);\
...
#define DECLARE_PSU_ATTR(index) \
    ...
    &sensor_dev_attr_psu##index##_fan1_input_max.dev_attr.attr, \
...
static struct attribute *as9957_32db_psu1_attrs[] = {
    /* psu attributes */
    DECLARE_PSU_ATTR(1),
...
static struct attribute_group as9957_32db_psu1_group = {
    .attrs = as9957_32db_psu1_attrs,
};
/* ATTRIBUTE_GROUPS(as9957_32db_psu1); */
...
static struct attribute *as9957_32db_psu2_attrs[] = {
    /* psu attributes */
    DECLARE_PSU_ATTR(2),
...
static struct attribute_group as9957_32db_psu2_group = {
    .attrs = as9957_32db_psu2_attrs,
};
/* ATTRIBUTE_GROUPS(as9957_32db_psu2); */

const struct attribute_group *as9957_32db_psu_groups[][2] = {
    {&as9957_32db_psu1_group, NULL},
    {&as9957_32db_psu2_group, NULL}
};

static struct as9957_32db_psu_data *as9957_32db_psu_update_device(struct device_attribute *da)
{
...
/**
 * psu_lookup_max_fan_speed - Lookup PSU max fan speed by model prefix.
 * @model: PSU model string (must be NUL-terminated).
 *
 * The mapping table uses prefix matching (strncmp with configured length).
 * Return: matched max fan speed, or 0 if not found.
 */
static u32 psu_lookup_max_fan_speed(const char *model)
{
    int i;
    ...
    for (i = 0; i < (int)ARRAY_SIZE(psu_max_fan_speed); i++) {
        size_t len = psu_max_fan_speed[i].length;

        if (len == 0) {
            continue;
        }

        if (strncmp(model, psu_max_fan_speed[i].model_name, len) == 0) {
            return psu_max_fan_speed[i].max_fan_speed;
        }
    }
                            ...
                            char *buf)
{
    ...
    data = as9957_32db_psu_update_device(da);
    if (!data->valid[pid]) {
        error = -EIO;
        goto exit;
    }
    ...
    case PSU2_FAN_INPUT:
        ...
        break;
    case PSU1_FAN_INPUT_MAX:
    case PSU2_FAN_INPUT_MAX:
        VALIDATE_PRESENT_RETURN(pid);

        value = data->ipmi_resp[pid].max_fan_speed;
        if (value == 0) {
               ...
               const char *buf, size_t count)
{
    ...
    status = kstrtouint(buf, 10, &speed);
    if (status)
        return status;
    ...
    case PSU2_FAN_INPUT_MAX:
        ...
        data->ipmi_resp[pid].max_fan_speed = speed;
                            ...
                            char *buf)
{
    ...
    case PSU2_TYPE:
        VALIDATE_PRESENT_RETURN(pid);
        value = data->ipmi_resp[pid].info[PSU_TYPE];
        break;
...
static int as9957_32db_psu_probe(struct platform_device *pdev)
{
    ...
    for(i = 0; i < 2; i++) {
        hwmon_dev = hwmon_device_register_with_info(&pdev->dev, DRVNAME, 
                        NULL, NULL, as9957_32db_psu_groups[i]);
        if (IS_ERR(hwmon_dev)) {
...
static int __init as9957_32db_psu_init(void)
{
    int ret;

    data = kzalloc(sizeof(struct as9957_32db_psu_data), GFP_KERNEL);
    if (!data) {
        ret = -ENOMEM;
        goto alloc_err;
    }
    ...
    ret = platform_driver_register(&as9957_32db_psu_driver);
    if (ret < 0)
        goto dri_reg_err;
...
dev_reg_err:
    platform_driver_unregister(&as9957_32db_psu_driver);
dri_reg_err:
    kfree(data);
alloc_err:
    return ret;
...
static void __exit as9957_32db_psu_exit(void)
{
...
MODULE_DESCRIPTION("as9957_32db_psu driver");
MODULE_LICENSE("GPL");

module_init(as9957_32db_psu_init);
module_exit(as9957_32db_psu_exit);
