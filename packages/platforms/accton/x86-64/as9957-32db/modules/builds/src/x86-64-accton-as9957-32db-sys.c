...
#define DRVNAME "as9957_32db_sys"
...
struct as9957_32db_sys_data *data = NULL;

static struct platform_driver as9957_32db_sys_driver = {
    .probe = as9957_32db_sys_probe,
    .remove = as9957_32db_sys_remove,
    .driver = {
        .name = DRVNAME,
        .owner = THIS_MODULE,
    },
...
enum as9957_32db_sys_sysfs_attrs {
    COME_CPLD_VER,
...
static const struct attribute_group as9957_32db_sys_group = {
    .attrs = as9957_32db_sys_attributes,
};
...
static struct as9957_32db_sys_data *as9957_32db_sys_update_ver(
                    struct device_attribute *da)
{
    ...
    case DC_SCM_CPLD_VER:
        tx_data_len = 1;
        data->ipmi_tx_data[0] = 0x6;
        break;

    case SYS_CPLD_VER:
        tx_data_len = 1;
        data->ipmi_tx_data[0] = 0x61;
        break;
                                ...
                                struct device_attribute *da, char *buf)
{
    ...
    data = as9957_32db_sys_update_ver(da);
    if (!data->valid) {
        error = -EIO;
        goto exit;
    }
    ...
    switch (attr->index) {
    case COME_CPLD_VER:
        code_base = data->ipmi_resp_cpld[0];
        cpu_pcba = data->ipmi_resp_cpld[1];
        major = data->ipmi_resp_cpld[2];
        minor = data->ipmi_resp_cpld[3];
        count = sprintf(buf, "%02X.%02X.%02X.%02X\n", code_base, cpu_pcba, major, minor);
        break;
...
static int as9957_32db_sys_probe(struct platform_device *pdev)
{
    ...
    /* Register sysfs hooks */
    status = sysfs_create_group(&pdev->dev.kobj, &as9957_32db_sys_group);
    if (status)
        goto exit;
...
static int as9957_32db_sys_remove(struct platform_device *pdev)
{
    ...
    sysfs_remove_group(&pdev->dev.kobj, &as9957_32db_sys_group);

    return 0;
}

static int __init as9957_32db_sys_init(void)
{
    int ret;

    data = kzalloc(sizeof(struct as9957_32db_sys_data), GFP_KERNEL);
    if (!data) {
        ret = -ENOMEM;
        goto alloc_err;
    }
    ...
    ret = platform_driver_register(&as9957_32db_sys_driver);
    if (ret < 0)
        goto dri_reg_err;
...
dev_reg_err:
    platform_driver_unregister(&as9957_32db_sys_driver);
dri_reg_err:
    kfree(data);
alloc_err:
    return ret;
...
static void __exit as9957_32db_sys_exit(void)
{
...
MODULE_DESCRIPTION("as9957_32db_sys driver");
MODULE_LICENSE("GPL");

module_init(as9957_32db_sys_init);
module_exit(as9957_32db_sys_exit);
